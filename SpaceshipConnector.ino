...
ButtonPressHandler button(RTC_BUTTON);
ClockAdjuster clockAdjuster(display.clock, state.dateTime, button, rtc);
...
void setup() {
    ...
    //now.date.dayOfWeek = 1;     // Понедельник
    //now.date.dayOfMonth = 18;   // Сегодняшний день
    //now.date.month = 6;         // Июнь
    //now.date.year = 24;         // 2024
    //now.time.hour = 12;
    //now.time.minute = 32;
    //now.time.second = 0;
...
void loop() {
    ...
    if (!clockAdjuster.isAdjusting()) {
        const auto dateTime = rtc.getDateTime();
        state.dateTime = dateTime;
    }
    
    clockAdjuster.update(state.tick);
    updateDisplay();
    ...
    //Serial.print("Time: ");
    //Serial.print(state.dateTime.time.hour);
    //Serial.print(":");
    //Serial.print(state.dateTime.time.minute);
    //Serial.print(":");
    //Serial.println(state.dateTime.time.second);
}

static bool isUsbConnected() {
    return false; //Serial.dtr() && state.usbFrameCounter != getUsbFrameCounter();
}
...
static void updateDisplay() {
    ...
    if (clockAdjuster.isAdjusting()) {
        clockAdjuster.update(state.tick);
        display.text.showString(1, F("SET TIME"));
    } else if (audio.isSwitchedOn()) {
    ...
    // HVAC отрисовывается ВСЕГДА (даже во время настройки времени)
    ...
    if (hvacState.connected) {
        ...
        switch (hvacState.airDirection) {
            case 1: front = true; break;
            case 2: front = true; bottom = true; break;
            case 3: bottom = true; break;
            case 4: windshield = true; bottom = true; break;
            case 5: windshield = true; break;
        }
